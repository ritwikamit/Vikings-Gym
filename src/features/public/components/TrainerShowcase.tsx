...
export function TrainerShowcase() {
  const trainers = [
    { name: 'Vikram Singh', role: 'Head Coach', spec: 'Strength & Conditioning', exp: '8+ Yrs', color: '#0EA5E9' },
    { name: 'Ankit Kumar', role: 'Elite Trainer', spec: 'Bodybuilding Specialist', exp: '6+ Yrs', color: '#8B5CF6' },
    ...
    <section className="section-padding bg-black">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {trainers.map((t) => (
            <motion.div key={t.name} variants={staggerItem} className="h-full">
              <TiltCard intensity={15} className="h-full">
                <div
                  className="group h-full relative rounded-[2rem] overflow-hidden bg-[#0A0A0A] border border-white/5 hover:border-[#0EA5E9]/40 transition-all duration-500"
                >
                  <div className="relative h-80 bg-gradient-to-b from-[#161616] to-[#0A0A0A] flex flex-col items-center justify-center overflow-hidden">
                    ...
                    <div className="absolute inset-0 opacity-10 group-hover:opacity-20 transition-opacity duration-500">
                      ...
                      <div className="absolute bottom-10 right-10 w-60 h-60 border-2 border-[#0EA5E9] rounded-full" />
                    </div>
                    
                    <div className="relative w-32 h-32 rounded-full border-4 border-[#0EA5E9]/20 group-hover:border-[#0EA5E9] transition-all duration-500 flex items-center justify-center bg-black shadow-2xl overflow-hidden mb-6">
                      <span className="text-4xl font-black text-white">{getInitials(t.name)}</span>
                      <div className="absolute inset-0 bg-gradient-to-tr from-[#0EA5E9]/20 to-transparent" />
                    </div>

                    <div className="text-center px-6">
                      <span className="text-[10px] font-bold tracking-[0.3em] text-[#0EA5E9] uppercase mb-1 block">{t.role}</span>
                    ...
                    <div className="absolute inset-0 bg-black/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center translate-y-full group-hover:translate-y-0">
                      <Magnetic strength={0.4}>
                        <Link href="/trainers" className="block px-8 py-3 rounded-xl bg-[#0EA5E9] text-white text-xs font-bold tracking-widest uppercase hover:scale-105 transition-transform shadow-xl shadow-[#0EA5E9]/20">
                          View Full Profile
                        </Link>
                      </Magnetic>
                    </div>
        ...
        <Reveal className="text-center mt-12">
          <Link href="/trainers" className="inline-flex items-center gap-2 text-[#737373] hover:text-[#0EA5E9] text-sm font-bold tracking-widest uppercase transition-all group">
