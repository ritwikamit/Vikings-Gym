...
export function ContactSection() {
  ...
  const cards = [
    { icon: Phone, label: 'Hotline', value: GYM_INFO.phone, href: `tel:${GYM_INFO.phone}`, color: '#0EA5E9' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
          ...
          <Reveal direction="left">
            <div className="p-8 sm:p-12 rounded-[2.5rem] glass-card relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#0EA5E9] to-transparent" />
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-[#A3A3A3] uppercase tracking-widest ml-1">Full Name</label>
                     <input type="text" required value={formState.name} onChange={(e) => setFormState(s => ({ ...s, name: e.target.value }))}
                      className="w-full px-5 py-4 rounded-xl bg-white/[0.03] border border-white/10 text-white text-sm focus:outline-none focus:border-[#0EA5E9] focus:bg-white/[0.05] transition-all" placeholder="John Doe" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-[#A3A3A3] uppercase tracking-widest ml-1">Email Address</label>
                     <input type="email" required value={formState.email} onChange={(e) => setFormState(s => ({ ...s, email: e.target.value }))}
                      className="w-full px-5 py-4 rounded-xl bg-white/[0.03] border border-white/10 text-white text-sm focus:outline-none focus:border-[#0EA5E9] focus:bg-white/[0.05] transition-all" placeholder="john@example.com" />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-[#A3A3A3] uppercase tracking-widest ml-1">Phone Number</label>
                   <input type="tel" value={formState.phone} onChange={(e) => setFormState(s => ({ ...s, phone: e.target.value }))}
                     className="w-full px-5 py-4 rounded-xl bg-white/[0.03] border border-white/10 text-white text-sm focus:outline-none focus:border-[#0EA5E9] focus:bg-white/[0.05] transition-all" placeholder="+91 XXXXX XXXXX" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-[#A3A3A3] uppercase tracking-widest ml-1">Your Message</label>
                   <textarea required rows={4} value={formState.message} onChange={(e) => setFormState(s => ({ ...s, message: e.target.value }))}
                     className="w-full px-5 py-4 rounded-xl bg-white/[0.03] border border-white/10 text-white text-sm focus:outline-none focus:border-[#0EA5E9] focus:bg-white/[0.05] transition-all resize-none" placeholder="How can we help you conquer your goals?" />
                </div>
                <button type="submit" disabled={isSubmitting} className="w-full flex items-center justify-center gap-3 py-5 rounded-2xl font-black text-sm tracking-[0.2em] uppercase text-white bg-[#0EA5E9] hover:bg-[#0284C7] transition-all duration-300 shadow-xl shadow-[#0EA5E9]/20 disabled:opacity-50">
          ...
          <Reveal direction="right">
            <div className="space-y-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {cards.map((card) => (
                  <div key={card.label} className="group p-6 rounded-3xl bg-[#0A0A0A] border border-white/5 hover:border-[#0EA5E9]/30 transition-all duration-500">
                    <div className="w-12 h-12 rounded-2xl bg-white/[0.03] flex items-center justify-center mb-5 group-hover:bg-[#0EA5E9]/10 transition-colors shadow-lg">
                      <card.icon className="w-6 h-6 text-[#0EA5E9]" />
                    </div>
                    <p className="text-[#737373] text-[10px] uppercase tracking-[0.2em] font-bold mb-2">{card.label}</p>
                    {card.href ? (
                      <a href={card.href} className="text-white text-base font-bold hover:text-[#0EA5E9] transition-colors whitespace-pre-line leading-relaxed">{card.value}</a>
                    ) : (
