      ...
      <div className="glass rounded-xl p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm text-gray-400">
              ...
              <span>•</span>
              <span className="bg-[#1A1A1A] px-2 py-1 rounded text-xs border border-[#333]">{WORKOUT_PLAN.level}</span>
              <span className="bg-[#1A1A1A] px-2 py-1 rounded text-xs border border-[#333]">{WORKOUT_PLAN.goal}</span>
            </div>
          </div>
          
          <button className="flex items-center gap-2 px-6 py-3 bg-red-600 text-white rounded-lg font-medium hover:bg-red-700 transition w-full md:w-auto justify-center">
            <Play className="w-5 h-5 fill-current" /> Start Today's Workout
          </button>
        </div>

        <div className="flex overflow-x-auto gap-4 pb-4 mb-6 border-b border-[#333]">
          {["Mon - Push", "Tue - Pull", "Wed - Rest", "Thu - Legs", "Fri - Upper", "Sat - Lower", "Sun - Rest"].map((day, i) => (
            <button 
              ...
              className={`whitespace-nowrap px-4 py-2 rounded-lg text-sm font-medium transition ${
                i === 0 ? "bg-red-600 text-white" : "text-gray-400 hover:bg-white/5"
              }`}
            >
              {day}
            </button>
        ...
        <div className="space-y-4">
          ...
          {WORKOUT_PLAN.exercises.map((exercise, index) => (
            <div key={exercise.id} className="bg-[#1A1A1A] border border-[#333] rounded-lg p-4 flex flex-col md:flex-row justify-between gap-4 group hover:border-[#444] transition">
              <div className="flex items-start gap-4">
                <div className="w-8 h-8 rounded-full bg-[#222] text-gray-400 flex items-center justify-center font-bold shrink-0">
                  {index + 1}
                </div>
                <div>
              ...
              <div className="flex gap-4 md:gap-8 items-center border-t border-[#333] md:border-t-0 pt-4 md:pt-0">
                <div className="text-center">
                  <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Sets</p>
                  <p className="text-lg font-bold text-white">{exercise.sets}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Reps</p>
                  <p className="text-lg font-bold text-white">{exercise.reps}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Rest</p>
