    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">Manage potential members and inquiries.</p>
        </div>
      ...
      <div className="flex overflow-x-auto gap-4 pb-4">
        {STAGES.map((stage) => (
          <div key={stage} className="min-w-[280px] bg-[#141414] border border-[#262626] rounded-xl flex flex-col max-h-[80vh]">
            <div className="p-3 border-b border-[#262626] flex justify-between items-center bg-[slate-800] rounded-t-xl">
              <h3 className="font-semibold text-white text-sm uppercase tracking-wide">{stage}</h3>
              <span className="bg-[slate-700] text-xs px-2 py-0.5 rounded-full text-white">
                {MOCK_LEADS.filter((l) => l.stage === stage).length}
              </span>
            </div>
            <div className="p-3 flex-1 overflow-y-auto space-y-3">
              {MOCK_LEADS.filter((l) => l.stage === stage).map((lead) => (
                <div key={lead.id} className="bg-[slate-800] border border-[slate-700] p-3 rounded-lg hover:border-sky-500/50 transition cursor-pointer">
                  ...
                  <div className="flex justify-between items-center mt-3 pt-2 border-t border-[slate-700]">
                    <span className="text-[10px] uppercase tracking-wider text-slate-500">{lead.source}</span>
                  </div>
                </div>
              ))}
            </div>
